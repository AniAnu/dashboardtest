...
if(isset($_POST["data"])){
	...
	for($i=0; $i<count($json_aray); $i++){
		$sql = "INSERT INTO `smartsearch`.`user_data` (`name`, `status`, `group`) VALUES ('".$json_aray[$i]->name."', '".$json_aray[$i]->status."', '".$json_aray[$i]->group."')";
		$result = go_mysql($sql);
	}
...
$result = go_mysql($sql);
	...
	for($i=0; $i<count($input_row_array); $i++){
		$sql = "INSERT INTO `smartsearch`.`". $tablename ."` (`Rangees`) VALUES ('".$input_row_array[$i]."')";
		$result = go_mysql($sql);
	}
...
elseif(isset($_POST["fileupload"])){
	...
	$sql = "DELETE FROM fileupload";
	$result = go_mysql($sql);
	$sql = "INSERT INTO `smartsearch`.`fileupload` (`contant`,`str_con`) VALUES ('".$json_string."', '".$str_contant."')";
	$result = go_mysql($sql);
}
elseif(isset($_POST["input_radio_field"])){
	...
	$sql = "INSERT INTO `smartsearch`.`chart_js` (`labels`,`mlabels`,`data`) VALUES ('".$input_radio_field_value."', '".$input_textarea_field_value."', '".$input_chartjs_value."')";
	$result = go_mysql($sql);
}
